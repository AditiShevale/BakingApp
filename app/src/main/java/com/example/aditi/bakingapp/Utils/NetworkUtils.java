package com.example.aditi.bakingapp.Utils;

import android.net.Uri;
import android.util.Log;

import com.example.aditi.bakingapp.Pojo.Ingredients;
import com.example.aditi.bakingapp.Pojo.Recepie;
import com.example.aditi.bakingapp.Pojo.Steps;
    ...
    public static ArrayList<Recepie> extractFeaturesFromJson(String baking) throws JSONException {
        ...
        for (int i = 0; i < recipeArray.length(); i++) {
            ...
            for (int j = 0; j < ingredientJsonArray.length(); j++) {
                ...
                Ingredients ingredients = new Ingredients(ingredientQuantity, ingredientMeasure,
            ...
            for (int k = 0; k < stepsJsonArray.length(); k++) {
                ...
                Steps stepsObj = new Steps(stepsId, stepsSdescrp, stepsDescrp,
                        stepsVpath, stepsThumnail);

                stepsList.add(stepsObj);
            }

            Recepie recipeObj = new Recepie(recipeId, recipeName, ingredientList,
                    stepsList, recipeServings, recipeImage);
